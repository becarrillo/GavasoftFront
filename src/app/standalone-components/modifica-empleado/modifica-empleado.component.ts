...
export class ModificaEmpleadoComponent {
  ...
  modalToUpdateIsOpen : boolean = false;
  ...
  toggleModalToUpdate(isOpen: boolean) {
    const modalUpdateDialog = document.querySelector('#modalToUpdateElement');
    !isOpen ? (
      (modalUpdateDialog as HTMLDialogElement).showModal(),
      this.modalToUpdateIsOpen = true
    ) : (
      (modalUpdateDialog as HTMLDialogElement).close(),
      this.modalToUpdateIsOpen = false
    )
  }

  submitChanges(ev : Event) {
    ev.preventDefault();
    const modalDeleteDialog = document.querySelector('#modalToDeleteElement');
    !this.modalToUpdateIsOpen ? (
